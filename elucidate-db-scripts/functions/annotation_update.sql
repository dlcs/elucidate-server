...
CREATE OR REPLACE FUNCTION public.annotation_update(_collectionid character varying, _annotationid character varying, _json jsonb)
RETURNS SETOF annotation_get AS $BODY$
DECLARE
    _modifieddatetime timestamp without time zone DEFAULT now();
BEGIN
    UPDATE annotation SET json = _json, modifieddatetime = _modifieddatetime, cachekey = md5(_modifieddatetime::text) WHERE collectionid = (SELECT id FROM annotation_collection WHERE collectionid = _collectionid AND deleted = false ORDER BY modifieddatetime DESC LIMIT 1) AND annotationid = _annotationid AND deleted = false;
    RETURN QUERY SELECT a.annotationid, a.cachekey, ac.collectionid, a.createddatetime, a.deleted, a.json, a.modifieddatetime FROM annotation a LEFT JOIN annotation_collection ac ON a.collectionid = ac.id WHERE ac.collectionid = _collectionid  AND ac.deleted = false AND a.annotationid = _annotationid AND a.deleted = false ORDER BY modifieddatetime DESC LIMIT 1;
END; $BODY$
