...
CREATE OR REPLACE FUNCTION public.annotation_create(_collectionid character varying, _annotationid character varying, _json jsonb)
RETURNS SETOF annotation_get AS $BODY$
DECLARE
    _insertedid integer;
    _createddatetime timestamp without time zone DEFAULT now();
BEGIN
    INSERT INTO annotation(annotationid, collectionid, json, createddatetime, deleted, cachekey) VALUES (_annotationid, (SELECT id FROM annotation_collection WHERE collectionid = _collectionid AND deleted = false ORDER BY modifieddatetime DESC LIMIT 1), _json, _createddatetime, false, md5(_createddatetime::text)) RETURNING id INTO _insertedid;
    RETURN QUERY  SELECT a.annotationid, a.cachekey, ac.collectionid, a.createddatetime, a.deleted, a.json, a.modifieddatetime FROM annotation a LEFT JOIN annotation_collection ac ON a.collectionid = ac.id WHERE a.id = _insertedid;
END;$BODY$
